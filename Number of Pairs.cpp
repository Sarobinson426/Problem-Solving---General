...
int divisibleSumPairs(int n, int k, vector<int> ar) {
	int count, comparator, index;

	count = 0;
	for (vector<int>::iterator it = ar.begin(); it != ar.end(); it++) {
		comparator = *it;

		for (index = 0; index < ar.size(); index++) {
			if (comparator < ar[index]) {
				if (((comparator + ar[index]) % k) == 0) {
					//cout << comparator << " : " << ar[index] << endl;
					count++;
				}
			}
		}
...
int main()
{
	...
	n = 6;
	k = 22;
	result = divisibleSumPairs(n, k, v);
	cout << result << endl;
